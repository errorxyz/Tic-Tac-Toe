...
#sets window size and creates window
disp = 600
...
port = 1234#OPTIONAL PORT
s.connect((host, port))#connects to server

#refer server file for more detailed documentation
...
def draw(pos):#drawing x or o
	...
	if not pos in usedXPos and not pos in usedOPos: #if input is not already used then
                turn += 1
                if turn % 2 == 0:
                        dX(pos)
			...
			if turn % 2 == 0:
				if event.type == pygame.KEYDOWN:
					if event.key == pygame.K_q:
						...
						quit()
						s.close()
					elif event.key == pygame.K_1:#if input is 1
						draw(1)
				...
				position = int(str(s.recv(1024)).replace("b","").replace("'",""))
				draw(position)
